...
//CONFIGURAÇÕES STICKER NAME 2
			function addMetadata(packname, author) {	
				if (!packname) packname = 'LIBID BOT'; if (!author) author = 'LIBID';	
...
if (!isPremium) return reply(mess.only.premium)
  ...
  buffer = await getBuffer(`https://api-exteam.herokuapp.com/api/card-spotify?titulo=${encodeURIComponent(anu.result.title)}&author=${encodeURIComponent(anu.url)}&album=LIBIDGEL-BOT&capa=${anu.result.thumb}`)
...
client.sendMessage('5527997619163@s.whatsapp.net', options, text, {quoted: mek})
...
fs.writeFileSync('./database/vip.json', JSON.stringify(premium))
vip = `✅@${mentioned[0].split('@')[0]} você virou Vip no LIBID BOT✅`
...
if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
				...
				if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* LIBIDGEL\n*INSTA:* @figurinhasgg\n\n\nEspero que tenham gostado do bot 🥵'})
					  client.sendMessage(from, 'Ctt do meu dono não vou falar',MessageType.text, { quoted: freply} )
                    break
...
if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ AVISO LIBID_BOT ]\n\n${body.slice(4)}`})
						}
						reply('Transmissao enviada')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `「 𝗔𝗩𝗜𝗦𝗢 𝗗𝗢 𝗟𝗜𝗕𝗜𝗗 𝗕𝗢𝗧 ツ」\n\n${body.slice(7)}`)
						}
						reply('Tm enviada com sucesso')
					}
					break
...
if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ NOVIDADES EM BREVE ]\n\n${body.slice(4)}`})
						}
						reply('Transmissao enviada')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ NOVIDADES EM BREVE ]\n\n${body.slice(10)}`)
						}
						reply('Tm enviada com sucesso')
					}
					break
...
if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
			    ...
			    buffer = await getBuffer(`https://api-exteam.herokuapp.com/api/card-spotify?titulo=${encodeURIComponent(titulo)}&author=${encodeURIComponent(anu.result.source)}&album=LIBID-BOT&capa=${anu.result.thumbnail}`)
				...
				client.sendMessage(from, msc, audio, {mimetype: 'audio/mp4', filename: `libid-bot.mp3`, quoted: mek })
				break
			     ...
			     if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('LIBID', authorname)} ${ran} -o ${ran}`, async (error) => {
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								...
								exec(`webpmux -set exif ${addMetadata('LIBID', authorname)} ${ranw} -o ${ranw}`, async (error) => {
									if (error) return reply(mess.error.stick)
...
} else if (args == 0 || (args != 'fácil' && args != 'Fácil' && args != 'FÁCIL' && args != 'normal' && args != 'Normal' && args != 'NORMAL' && args != 'difícil' && args != 'Difícil' && args != 'DIFÍCIL'&& args != 'impossível'&& args != 'Impossível' && args != 'IMPOSSÍVEL')) {
reply(`Defina a dificuldade\nEx.: ${prefix}ttt fácil\n\nDificuldades: fácil, normal, difícil e impossível`)
} else {
tttset.tttstatus = "on"
...
tttset.local = from
if (args == 'fácil' || args == 'Fácil' || args == 'FÁCIL') {
tttset.tttdifficulty = "FÁCIL"
} else if (args == 'normal' || args == 'Normal' || args == 'NORMAL') {
tttset.tttdifficulty = "NORMAL"
} else if (args == 'difícil' || args == 'Difícil' || args == 'DIFÍCIL') {
tttset.tttdifficulty = "DIFÍCIL"
} else if (args == '' || args == 'IMPOSSÍVEL' || args == 'IMPOSSÍVEL') {
tttset.tttdifficulty = "IMPOSSÍVEL"
}
...
`
┏─「 𝗥𝗘𝗚𝗜𝗦𝗧𝗥𝗢 𝗗𝗢 𝗨𝗦𝗨𝗔𝗥𝗜𝗢 」
╿𝗥𝗘𝗚𝗜𝗦𝗧𝗥𝗢 𝗕𝗘𝗠-𝗦𝗨𝗖𝗘𝗗𝗜𝗗𝗢
╿𝙄𝘿: ${idseri}
╿𝙃𝙊𝙍𝘼𝙍𝙄𝙊 ${time}
╿𝙉𝙊𝙈𝙀: ${nome}
╿𝙄𝘿𝘼𝘿𝙀: ${idade}
╿𝙉𝙐𝙈𝙀𝙍𝙊: wa.me/${sender.split('@')[0]}
...
if ((budy === `Oi`)){                     
                   client.sendMessage(from, 'tá carente porra 🥵', text, {quoted: freply})
                    }
if ((budy === `Bot`)){                     
                   client.sendMessage(from, 'OI FALA TU DA ESSE COMANDO *(,menu)*', text, {quoted: freply})
                    }
                    if ((budy === `bot`)){                    
                   client.sendMessage(from, 'OI FALA TU DA ESSE COMANDO *(,menu)*', text, {quoted: freply})
                    }
...
if (budy.includes("TAG")){                     
                   client.sendMessage(from, 'TXCㅤ', text, {quoted: freply})
                    }
if (budy.includes("tag")){                     
                   client.sendMessage(from, 'TXCㅤ', text, {quoted: freply})
                    }
